...
func (s *Service) Create(ctx context.Context, command CreateCommand) (*domain.RemediationCase, error) {
	...
	value, err := s.coordinator.Do(ctx, command.CaseID, func(runCtx context.Context) (any, error) {
		...
		result, err := s.store.Create(runCtx, c, event, command.RequestID, fingerprint)
		if err != nil {
			return nil, err
		}
...
func (s *Service) commit(ctx context.Context, caseID, operation string, meta CommandMeta, body any, mutation store.Mutation, builder store.ArchiveBuilder) (*domain.RemediationCase, error) {
	...
	value, err := s.coordinator.Do(ctx, caseID, func(runCtx context.Context) (any, error) {
		return s.store.Commit(runCtx, caseID, meta.ExpectedRevision, meta.RequestID, fp, mutation, builder)
	})
	if err != nil {
		return nil, err
	}
