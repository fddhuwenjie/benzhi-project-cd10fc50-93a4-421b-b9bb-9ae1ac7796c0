...
func (c *Coordinator) Do(ctx context.Context, caseID string, run operation) (any, error) {
	worker, err := c.worker(caseID)
	if err != nil {
		return nil, err
	}
	result := make(chan jobResult, 1)
	item := job{ctx: ctx, run: run, result: result}
	select {
...
func (c *Coordinator) worker(caseID string) (*caseWorker, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	w := &caseWorker{jobs: make(chan job, c.queueSize), stop: make(chan struct{})}
	c.workers[caseID] = w
	go c.serve(caseID, w)
	return w, nil
}
...
func (c *Coordinator) serve(caseID string, worker *caseWorker) {
	...
	for {
		...
		case item := <-worker.jobs:
			...
			if item.ctx.Err() != nil {
				item.result <- jobResult{err: item.ctx.Err()}
			} else {
				value, err := item.run()
